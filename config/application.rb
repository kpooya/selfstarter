...
LINQUET_SERVER_URL_ENCODED = URI.encode(LINQUET_SERVER_URL)
LINQUET_MINI_URL = SetLinuqetUrl::FRONT_SERVER_URL + '/mini'
LINQUET_MINI_URL_ENCODED = URI.encode(LINQUET_MINI_URL)
...
class SubPath
  ...
  def self.getUrl(path)
    LINQUET_SERVER_URL + get_frontend_path(path)
  end
end
